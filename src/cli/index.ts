...
async function main() {
  ...
  program
    ...
    .option("--min-stars <n>", "minimum star count", "0")
    .option("--since <date>", "pushed after date, YYYY-MM-DD")
    .option("--sort <field>", "stars | updated | forks", "stars")
    ...
    .action(async (query, options) => {
      ...
      const sort = ["stars", "updated", "forks"].includes(options.sort)
        ? options.sort
        : "stars";
      ...
      const command = new SearchCommand(githubAdapter, analyzeRepo);
